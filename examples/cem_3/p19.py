...
def step_initial_condition(problem_data, x):
    """
...
def plot_solution(problem, sp, u_h) -> None:
    ...
    axs.set_xlabel(fr"$x/L$")
    axs.set_ylabel(r"$u(x)$")
    axs.set_xlim(xmin, xmax / x_scale)
    axs.set_ylim(-0.5, 1.5)
    axs.set_xticks(np.linspace(xmin, xmax / x_scale, 11))
    axs.legend(
        loc='center left',
...
def single_test_solution(problem) -> None:
    ...
    step = step_initial_condition(problem, sp.x)
    driver = LinAdvecDriver1D(problem, sp, u0 = step)
