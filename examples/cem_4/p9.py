...
PML_DESIGN = {
    'x0': 1.0,          # Posição inicial da camada de PML na direção x
    'L': 1.0,           # Largura da camada de PML
    'pml_order': 2,     # Ordem polinomial do PML
...
class SpectralAnalyzer:
    ...
    def single_test(self, SpectralAnalyzer, sp):
        """
        Teste simples da classe SpectralAnalyzer.
        """
        # Inicializa o driver Maxwell com o problema definido    
        driver = MaxwellDriver(sp, CFL=PROBLEM['cfl'])
        driver['Ez'][:] = SpectralAnalyzer.gaussian_pulse(x=sp.x, y=sp.y, t=0)
        
        print(f"\n🌐 Inicializando o driver Maxwell com CFL = {PROBLEM['cfl']:.2f} e dt = {driver.dt:.3f} s")
        ...
        self.plot_griddata_global_solution(driver, field_label='Ez', N_grid=300)
    ...
    def plot_analytical_time_evolution(self, N_grid=200):
        """
    ...
    def plot_griddata_global_solution(self, driver, field_label='Ez', N_grid=300):
        """
        ...
        x = driver.sp.x.ravel(order='F')
        y = driver.sp.y.ravel(order='F')
        u = driver[field_label].ravel(order='F')
        ...
        # Gráficos
        fig = plt.figure(figsize=(14, 6))
        fig.suptitle(fr'Interpolação com `griddata` da solução ${field_label}(x, y, t={{driver.timeIntegrator.time:.3f}} s)$', fontsize=14)
    ...
    def plot_field_evolution_on_domain(self, SpectralAnalyzer, sp, N_grid=300):
        """
        ...
        # Inicializa o driver Maxwell com o problema definido    
        driver = MaxwellDriver(sp, CFL=PROBLEM['cfl'])
        driver['Ez'][:] = SpectralAnalyzer.gaussian_pulse(sp.x, sp.y, 0)
    ...
    def plot_field_evolution_on_x_axis(self, sa, sp, N_grid=500):
        ...
        # Obter os limites da PML a partir do dicionário de configuração
        pml_interface = PML_DESIGN['x0']

        for i, (ez_data, t) in enumerate(zip(all_ez_on_axis, T_LIST)):
            ax = axes[i]

            # >>> NOVIDADE: Colorir as regiões da PML <<<
            # Colore a PML da direita (x > interface)
            ax.axvspan(pml_interface, x_max, color='gray', alpha=0.3, zorder=0)
            # Colore a PML da esquerda (x < -interface)
            ax.axvspan(x_min, -pml_interface, color='gray', alpha=0.3, zorder=0)
    ...
    def plot_field_at_physical_interface(self, SpectralAnalyzer, sp, n_samples=500):
        """
        ...
        sp = driver.sp
        distances = np.sqrt((sp.x - 1.0)**2 + (sp.y - 0.0)**2)
        node_idx, elem_idx = np.unravel_index(np.argmin(distances), distances.shape)

        print(f"\nMonitoring field at node closest to (1, 0):")
        ...
        plt.plot(times, ez_values, label=r'$E_z(x=1, y=0, t)$')
        plt.xlabel("Time (s)")
        plt.ylabel(r'$E_z$ (V/m)')
        plt.title(r'Electric Field Evolution at PML Interface')
...
def main() -> None:
    ...
    mesh_data = mesh_rectangular_pml_domain(PROBLEM, PML_DESIGN, BOUNDARY, MATERIAL, h=0.5, view_mesh=False)
    ...
    # Visualizar a malha criada e solução analítica
    sa.plot_collocation_points()
    # sa.plot_analytical_time_evolution()
    # sa.plot_analytical_field_at_final_time()

    # Initialize the solver
    sp = Maxwell2D(n_order=PROBLEM['n_order'], mesh=pml_domain, fluxType=PROBLEM['flux_type'], pml_design=PML_DESIGN)

    # --- Perfil de condutividade da camada PML ---
    # sa.plot_sigma_surfaces(sp)
    
    # --- Teste simples da classe SpectralAnalyzer ---
    # sa.single_test(sa, sp)

    # --- Coletar snapshots ao longo do tempo ---
    sa.plot_field_evolution_on_x_axis(sa, sp)
    sa.plot_field_evolution_on_domain(sa, sp)
    sa.plot_field_at_physical_interface(sa, sp)
    plt.show()
